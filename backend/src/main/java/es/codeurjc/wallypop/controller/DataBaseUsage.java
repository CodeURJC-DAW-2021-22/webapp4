	...
	public void run(String... args) throws Exception {
		// User
				...
				articleService.save(new Article(userService.findById(1).get(), "Zapatillas deportivas", "Descripción",
							"Madrid", (float) 99.99, convertToBLOB("/sample_image/ropa.png"), categoryService.findById(2).get()));
				articleService.save(new Article(userService.findById(2).get(), "Zapatillas correr", "Descripción", "Madrid",
							(float) 50.99, convertToBLOB("/sample_image/ropa.png"), categoryService.findById(2).get()));			
				articleService.save(new Article(userService.findById(1).get(), "Botas montaña", "Deja que tus pies descanses mientras caminas", "Extremadura",
						(float) 59.99, convertToBLOB("/sample_image/botasMontana.jpg"), categoryService.findById(2).get()));
				articleService.save(new Article(userService.findById(1).get(), "Cestas de mimbre", "Cestas de mimbre hechas a mano, con asa muy resistente", "Canarias",
						(float) 8.99, convertToBLOB("/sample_image/cestaMimbre.jpg"), categoryService.findById(3).get()));
				articleService.save(new Article(userService.findById(1).get(), "Jarrones", "Jarrones de cerámica hechos a mano, con colores vivos", "Teruel",
						(float) 8.99, convertToBLOB("/sample_image/jarron.jpg"), categoryService.findById(3).get()));
				articleService.save(new Article(userService.findById(1).get(), "Llaves inglesas", "Se venden llaves inglesas con mango", "Barcelona",
						(float) 18.99, convertToBLOB("/sample_image/llaveInglesa.jpg"), categoryService.findById(4).get()));
				articleService.save(new Article(userService.findById(1).get(), "Kit electrónica", "Se vende kit de electrónica para que puedas arreglar tus dispositivos electrónicos como móviles, ordenadores y todo tipo de dispositivos", "A Coruña",
						(float) 14.99, convertToBLOB("/sample_image/kitElectronica.jpg"), categoryService.findById(5).get()));
				articleService.save(new Article(userService.findById(1).get(), "Teclados", "Teclado castellano. Tu imaginación no tiene límites", "Madrid",
						(float) 9.99, convertToBLOB("/sample_image/teclado.jpg"), categoryService.findById(5).get()));
				articleService.save(new Article(userService.findById(1).get(), "Ratones", "Ratones inalámbricos muy veloces. Deja que tu puntero apunte por ti", "Cuenca",
						(float) 4.99, convertToBLOB("/sample_image/raton.jpg"), categoryService.findById(5).get()));
				articleService.save(new Article(userService.findById(1).get(), "Saco de dormir", "Se venden sacos de dormir muy confortables, para que no pases frio durante la noche", "Madrid",
						(float) 29.99, convertToBLOB("/sample_image/sacoDormir.jpg"), categoryService.findById(6).get()));
				articleService.save(new Article(userService.findById(1).get(), "Linterna", "Se venden linternas ligeras a buen precio, viene sin dos pilas AAA", "Aragón",
						(float) 4.99, convertToBLOB("/sample_image/linterna.jpg"), categoryService.findById(6).get()));
				articleService.save(new Article(userService.findById(1).get(), "Colonia Aquarel", "Se venden perfume Aquarel para mujer a buen precio, el olor dura mucho tiempo", "Asturias",
						(float) 11.99, convertToBLOB("/sample_image/perfume.jpg"), categoryService.findById(7).get()));
				articleService.save(new Article(userService.findById(1).get(), "Colonia para niños", "Se venden colonias para niños a buen precio", "Granada",
						(float) 4.99, convertToBLOB("/sample_image/perfumeNino.jpg"), categoryService.findById(7).get()));
				*/
